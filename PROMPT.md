...
For Claude, paste **everything inside the code block** into Project instructions. For ChatGPT, add
this file as a Project source and use the short Project instruction in [SETUP.md](SETUP.md). For
Codex, install the folder as an agent skill.

The code block is already filled with a fictional Marcus Webb setup so you can run the supplied
samples without editing anything. Replace all seven values under `<operator_config>` before using a
real statement.
...
<operator_config>
TEST SETUP ONLY. The values below are fictional. Replace all seven before a live run.
